...
export const SignIn = () => {
  ...
  const [isLoginSuccess, setIsLoginSuccess] = useState(user.isloggedInSuccess);
  ...
  const loginViaGoogle = async (token) => {
    ...
    const fetchUser = async () => {
      try {
        dispatch(USER_LOGIN_REQUEST());
        const response = await userApi.loginViaGoogle(jwtDecodeUser.email);
        localStorage.setItem("token", JSON.stringify(response));
        dispatch(USER_LOGIN_SUCCESS(response));
      } catch (error) {
        const errorMessage = getErrorMessageFromServer(error);
  ...
  useEffect(() => {
    ...
    if (user.token && user.userInfor.role === "ADMIN") navigate("/dashboard");
  }, [user.token]);
                      ...
                      >
                        <GoogleLogin
                          onSuccess={(credentialResponse) =>
                            loginViaGoogle(credentialResponse.credential)
                          }
                          onError={() => {
