                ...
                {Object.keys(product).length === 0
                  ...
                  : product.listImages.map((item, index) => (
                      <img
                        ...
                        onMouseOver={() => setProductImage(item.imgPath)}
                        key={index}
                      />
                    ))}
              </div>
            ...
            <div className="w-[40%] flex flex-col justify-between">
              <div>
                ...
                <p className="text-orange-500 font-trebu font-semibold text-lg mt-10">
                  Giá:{" "}
                  {convertPriceToString(
                    product.basePrice - product.basePrice * product.deal
                  )}{" "}
                  VNĐ{" "}
                  {product.deal === 0 || (
                    <span className="text-gray-400/60 text-base font-thin line-through ml-4">
                      {convertPriceToString(product.basePrice)} VNĐ
                    </span>
                  )}
                </p>
                <div className="flex mt-3">
                  <p className="w-11/12 text-gray-500">{product.description}</p>
                </div>
              </div>
              <div className="mb-20">
                ...
                <div className="mt-2 mb-1 flex">
                  <div className="flex">
                    <button
                      ...
                      onClick={() => {
                        if (!productAmount) return setProductAmount(1);
                        if (productAmount > 1)
                          return setProductAmount((pre) => pre - 1);
                      }}
                    >
                      -
                    </button>
